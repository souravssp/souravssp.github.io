(function(){
  ...
  var FaqTemplate = function(element) {
		...
		this.faqsCategoriesContainer = this.element.getElementsByClassName('cd-faq__group')[0];
		this.faqsCategories = this.faqsCategoriesContainer.getElementsByClassName('cd-faq__group');
  ...
  function initFaqEvents(faqs) {
  	// click on a faq category
		faqs.faqsCategoriesContainer.addEventListener('click', function(event){
			var category = event.target.closest('.cd-faq__group');
			if(!category) return;
			var mq = getMq(faqs),
